...
TickType_t tick_count1;
QueueHandle_t cJsonHandle1;
cJSON *root;
SemaphoreHandle_t root_mutex;
...
cJSON *mavlinkDataToJSON(MavlinkData *data) {
  ...
  printf("Pointer address mavlinkDataToJSON ONEMLI: %p\n", (void *)root);
...
void socket_listen(void) {
  ...
  if (bind(server_socket, (struct sockaddr *)&server_addr,
    ...
    ESP_LOGE(TAG, "Socket baglanamadi");
    close(server_socket);
    if (root != NULL) {
      cJSON_Delete(root); // root nesnesini serbest bırak
    }
  ...
  int client = 0;
  char BUFFER[1024];
  int err = listen(server_socket, 1);
  if (err != 0) {
  ...
  while (1) {
    ...
    timeout.tv_sec = 1; // 0.5 saniye
    timeout.tv_usec = 0;
    if (setsockopt(client, SOL_SOCKET, SO_SNDTIMEO, &timeout, sizeof(timeout)) <
        0) {
      ESP_LOGE(TAG, "Set socket send timeout failed: errno %d", errno);
      close(client);
      continue;
    }
    ...
    while (1) {
      if (xSemaphoreTake(root_mutex, portMAX_DELAY) == pdTRUE) {
        json_data = cJSON_PrintUnformatted(root);
        xSemaphoreGive(root_mutex);
      }
      if (json_data == NULL) {

        ESP_LOGE(TAG, "json_data is NULL, recreating root");
        if (root != NULL) {
          cJSON_Delete(root); // Eski root nesnesini serbest bırak
        }
        root = mavlinkDataToJSON(&data);
        printf("JSON data11111: %s\n",
               cJSON_PrintUnformatted(root)); // Yeni root nesnesini oluştur
        if (root == NULL) {
          ESP_LOGE(TAG, "Failed to recreate root");
          close(client);
          break;
        }
        json_data = cJSON_PrintUnformatted(root);
        if (json_data == NULL) {
          ESP_LOGE(TAG, "json_data is still NULL after recreating root");
          close(client);
          break;
        }
      }
      int bytes_sent = send(client, json_data, strlen(json_data), 0);
      if (bytes_sent < 0) {
        ESP_LOGE(TAG, "Send failed: errno %d", errno);
        break;
      }
      cJSON_free(json_data);
      TickType_t tick_count1 = xTaskGetTickCount();
      TickType_t tick_difference = tick_count1;
      uint32_t time_difference_ms = tick_difference * portTICK_PERIOD_MS;

      // Geçen zamanı saniye cinsinden hesaplayın
      float time_difference_s = time_difference_ms / 1000.0;

      ESP_LOGI(TAG, "Time difference bittiiiiii: %ld ms (%.2f s)",
               time_difference_ms, time_difference_s);
...
void print_esp32_ap_ip() {
  ...
  esp_netif_t *ap_netif =
      esp_netif_get_handle_from_ifkey("WIFI_AP_DEF"); // AP modunun netif'ini al
...
void handleMavlinkMessage(cJSON *root, const mavlink_message_t *msg) {
  printf("Pointer address handleMavlinkMessage: %p\n", (void *)root);
  switch (msg->msgid) {
  case MAVLINK_MSG_ID_HEARTBEAT: {
    mavlink_heartbeat_t heartbeat;
  ...
  case MAVLINK_MSG_ID_GPS_RAW_INT: {
    ...
    printf("GPS - Lat: %ld, Lon: %ld, Alt: %ld, Fix: %d\n", gps_raw.lat,
           gps_raw.lon, gps_raw.alt, gps_raw.fix_type);
    if (xSemaphoreTake(root_mutex, portMAX_DELAY) == pdTRUE) {
  ...
  case MAVLINK_MSG_ID_POWER_STATUS: {
    ...
    printf("Power Status - Vcc: %d, Vservo: %d, Flags: %d\n", power_status.Vcc,
           power_status.Vservo, power_status.flags);
    if (xSemaphoreTake(root_mutex, portMAX_DELAY) == pdTRUE) {
  ...
  case MAVLINK_MSG_ID_RAW_IMU: {
    ...
    printf("Raw IMU - Accel: [%d, %d, %d], Gyro: [%d, %d, %d]\n", raw_imu.xacc,
           raw_imu.yacc, raw_imu.zacc, raw_imu.xgyro, raw_imu.ygyro,
           raw_imu.zgyro);
    if (xSemaphoreTake(root_mutex, portMAX_DELAY) == pdTRUE) {
  ...
  case MAVLINK_MSG_ID_VIBRATION: {
    ...
    printf("Vibration Message Received:  Vibration X: %.2f, Vibration Y: %.2f, "
           "Vibration Z: %.2f, Clipping 0: %ld, Clipping 1: %ld, Clipping 2: "
           "%ld\n",
  ...
  case MAVLINK_MSG_ID_SERVO_OUTPUT_RAW: {
    ...
    printf(
        "Servo Output Raw - Servo1: %d, Servo2: %d, Servo3: %d, Servo4: %d\n",
        servo_output.servo1_raw, servo_output.servo2_raw,
  ...
  case MAVLINK_MSG_ID_TERRAIN_REPORT: {
    ...
    printf("Terrain Report - Lat: %ld, Lon: %ld, Terrain Height: %.2f\n",
           terrain_report.lat, terrain_report.lon,
...
void uart_recevier(void) {
  ...
  while (1) {
    ...
    // printf("Pointer address uart_recevier: %p\n", (void *)root);
      ...
      case UART_DATA:
        ...
        for (int i = 0; i < length; i++) {
          ...
          if (mavlink_parse_char(MAVLINK_COMM_0, data[i], &msg, &status)) {
            ...
            TickType_t tick_difference = tick_count;
            uint32_t time_difference_ms = tick_difference * portTICK_PERIOD_MS;

// Geçen zamanı saniye cinsinden hesaplayın
            float time_difference_s = time_difference_ms / 1000.0;

            ESP_LOGI(TAG, "Time difference BASLADIIII: %ld ms (%.2f s)",
time_difference_ms, time_difference_s);*/
          ...
          /*if (data[0] == 0xFD)
          {
              ...
              {
                  if (uart_read_bytes(UART_NUM_2, data+10, data[1]+ 2,
          portMAX_DELAY) == data[1]+ 2)
                  {
                      uart_mavlink(data);
                      break;
                  }
...
void app_main() {
  ...
  root_mutex = xSemaphoreCreateMutex();
  if (root_mutex == NULL) {
  ...
  wifi();
  print_esp32_ap_ip();
  xTaskCreatePinnedToCore(uart_recevier, "UART Receiver Task", 4096, NULL, 5,
                          NULL, 0);
  xTaskCreatePinnedToCore(socket_listen, "Socket Listen Task", 4096, NULL, 5,
                          NULL, 1);
}
