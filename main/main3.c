...
void socket_listen(void){
    ...
    while (1)
 {
    ...
    ESP_LOGI(TAG, "Time difference bittiiiiii: %ld ms (%.2f s)", time_difference_ms, time_difference_s);
...
void uart_recevier(void)
{
   ...
   case UART_DATA:
    ...
    // Geçen zamanı saniye cinsinden hesaplayın
                ...
                ESP_LOGI(TAG, "Time difference BASLADIIII: %ld ms (%.2f s)", time_difference_ms, time_difference_s);*/
...
void app_main(){
    ...
    wifi();
    print_esp32_ap_ip();
    xTaskCreatePinnedToCore(uart_recevier, "UART Receiver Task", 4096, NULL, 5, NULL, 0);
    xTaskCreatePinnedToCore(socket_listen, "Socket Listen Task", 4096, NULL, 5, NULL, 1);
}
